...
describe("MongoRepository", () => {
  ...
  async function seedTestCollection(
    properties: EntityProperties<MongoTestEntity>[] = []
  ) {
  ...
  describe("getById", () => {
    ...
    it("should return undefined when there is not an entity with the given id", async () => {
      ...
      const entity = await mongoRepository.getById(
        unexistentEntityId.toString()
      );
  ...
  describe("getByIds", () => {
    ...
    it("should return array with entities that match give ids", async () => {
      ...
      expect(foundEntities).toEqual(
        expect.arrayContaining([entities[0], entities[1]])
      );
    });
  });

  describe("getByFilter", () => {
    it("should get elements that are equal to given values", async () => {
      await seedTestCollection();

      const entities = await mongoRepository.getAll();

      const foundEntities = await mongoRepository.getByFilter({
        stringProperty: "AAA",
      });
    ...
    it("should get elements that does not have specific keys", async () => {
      const numberProperty = 123;
      const currentDate = new Date();

      await mongoTestContext.collection.insertMany([
        {
          createdAt: currentDate,
          updatedAt: currentDate,
          stringProperty: "AAA",
      ...
      await mongoRepository.deleteByFilter({
        numberProperty: undefined,
      });

      const remainingEntities = await mongoRepository.getAll();

      expect(remainingEntities.length).toEqual(1);
      expect(remainingEntities[0].numberProperty).toEqual(numberProperty);
    });
  });
  ...
  describe("deleteById", () => {
    it("should delete element by id", async () => {
      ...
      for (const entity of entities) {
        ...
        expect(
          remainingEntities.every((remainingEntity) => {
    ...
    it("should accept an unmatching id without throwing an error", async () => {
      ...
      const entity = await mongoRepository.deleteById(
        unexistentEntityId.toString()
      );
  ...
  describe("deleteByIds", () => {
    ...
    it("should delete elements by ids", async () => {
      ...
      await mongoRepository.deleteByIds([firstEntityId, secondEntityId]);

      const remainingEntities = await mongoRepository.getAll();

      expect(
        remainingEntities.every(({ id }) => {
  ...
  describe("deleteByFilter", () => {
    it("should delete elements that are equal to given values", async () => {
      await seedTestCollection();
      ...
      expect(
        remainingEntities.every(
          ({ stringProperty }) => stringProperty !== "AAA"
        )
      ).toEqual(true);
    });

    it("should delete elements that does not have specific keys", async () => {
      const numberProperty = 123;
      const currentDate = new Date();
  ...
  describe("updateById", () => {
    ...
    it("should unset entity properties", async () => {
      ...
      const updateEntityResult = await mongoRepository.updateById(
        someEntity.id,
        {
      ...
      expect(updateEntityResult.stringProperty).toEqual(
        someEntity.stringProperty
      );
      expect(updateEntityResult.numberProperty).toEqual(
        someEntity.numberProperty
      );
      expect(updateEntityResult.dateProperty).toEqual(someEntity.dateProperty);
      expect(updateEntityResult.booleanProperty).toEqual(
        someEntity.booleanProperty
      );
      ...
      expect(updatedEntity?.dateProperty).toEqual(someEntity.dateProperty);
      expect(updatedEntity?.booleanProperty).toEqual(
        someEntity.booleanProperty
      );
      expect(updatedEntity && "optionalStringProperty" in updatedEntity).toBe(
        false
      );
      expect(updatedEntity && "optionalNumberProperty" in updatedEntity).toBe(
        false
      );
      expect(updatedEntity && "optionalDateProperty" in updatedEntity).toBe(
        false
      );
      expect(updatedEntity && "optionalBooleanProperty" in updatedEntity).toBe(
        false
      );
    });
  });
  ...
  describe("updateByIds", () => {
    it("should update entities", async () => {
      ...
      await mongoRepository.updateByIds([someEntityId, anotherEntityId], {
        stringProperty: "ABC",
        numberProperty: 321,
        dateProperty: new Date(1000),
        booleanProperty: false,
      ...
      const someUpdatedEntity = updatedEntities.find(
        (e) => e.id === someEntityId
      );
      ...
      const anotherUpdatedEntity = updatedEntities.find(
        (e) => e.id === anotherEntityId
      );
    ...
    it("should partially update entities", async () => {
      ...
      const anotherUpdatedEntity = await mongoRepository.getById(
        anotherEntityId
      );
      ...
      expect(someUpdatedEntity?.stringProperty).toEqual("ABC");
      expect(someUpdatedEntity?.numberProperty).toEqual(321);
      expect(someUpdatedEntity?.dateProperty).toEqual(
        entities[0]?.dateProperty
      );
      expect(someUpdatedEntity?.booleanProperty).toEqual(
        entities[0]?.booleanProperty
      );

      expect(anotherUpdatedEntity?.id).toEqual(anotherEntityId);
      expect(anotherUpdatedEntity?.stringProperty).toEqual("ABC");
      expect(anotherUpdatedEntity?.numberProperty).toEqual(321);
      expect(anotherUpdatedEntity?.dateProperty).toEqual(
        entities[1]?.dateProperty
      );
      expect(anotherUpdatedEntity?.booleanProperty).toEqual(
        entities[1]?.booleanProperty
      );
    });
    ...
    it("should unset properties from many entities", async () => {
      ...
      const [someEntityUpdateResult, anotherEntityUpdateResult] =
        await mongoRepository.updateByIds([someEntity.id, anotherEntity.id], {
          optionalStringProperty: undefined,
          optionalNumberProperty: undefined,
          optionalDateProperty: undefined,
          optionalBooleanProperty: undefined,
      ...
      expect(
        someEntityUpdateResult &&
          "optionalStringProperty" in someEntityUpdateResult
      ).toBe(false);
      expect(
        someEntityUpdateResult &&
          "optionalNumberProperty" in someEntityUpdateResult
      ).toBe(false);
      expect(
        someEntityUpdateResult &&
          "optionalDateProperty" in someEntityUpdateResult
      ).toBe(false);
      expect(
        someEntityUpdateResult &&
          "optionalBooleanProperty" in someEntityUpdateResult
      ).toBe(false);
      ...
      expect(
        anotherEntityUpdateResult &&
          "optionalStringProperty" in anotherEntityUpdateResult
      ).toBe(false);
      expect(
        anotherEntityUpdateResult &&
          "optionalNumberProperty" in anotherEntityUpdateResult
      ).toBe(false);
      expect(
        anotherEntityUpdateResult &&
          "optionalDateProperty" in anotherEntityUpdateResult
      ).toBe(false);
      expect(
        anotherEntityUpdateResult &&
          "optionalBooleanProperty" in anotherEntityUpdateResult
      ).toBe(false);

      const [someUpdatedEntity, anotherUpdatedEntity] =
      ...
      expect(
        someUpdatedEntity && "optionalStringProperty" in someUpdatedEntity
      ).toBe(false);
      expect(
        someUpdatedEntity && "optionalNumberProperty" in someUpdatedEntity
      ).toBe(false);
      expect(
        someUpdatedEntity && "optionalDateProperty" in someUpdatedEntity
      ).toBe(false);
      expect(
        someUpdatedEntity && "optionalBooleanProperty" in someUpdatedEntity
      ).toBe(false);
      ...
      expect(
        anotherUpdatedEntity && "optionalStringProperty" in anotherUpdatedEntity
      ).toBe(false);
      expect(
        anotherUpdatedEntity && "optionalNumberProperty" in anotherUpdatedEntity
      ).toBe(false);
      expect(
        anotherUpdatedEntity && "optionalDateProperty" in anotherUpdatedEntity
      ).toBe(false);
      expect(
        anotherUpdatedEntity &&
          "optionalBooleanProperty" in anotherUpdatedEntity
      ).toBe(false);
    });
  });
  ...
  describe("createMany", () => {
    it("should create many entities", async () => {
      ...
      const foundEntities = await mongoRepository.getByIds(
        createdEntities.map((entity) => entity.id)
      );
